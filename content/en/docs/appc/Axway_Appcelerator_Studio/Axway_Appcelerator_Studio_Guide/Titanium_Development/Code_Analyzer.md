...
{{% alert title="⚠️ Warning" color="primary" %}}If **Process Univisited Code** is enabled, you may receive a lot of false positives and inaccurate results due to platform-specific code being visited and functions being called that do not exist.

For example, if enabling this option for an Alloy project, errors will be reported as a result of processing files like Resources/alloy/backbone.js, which is generated by the Alloy compiler, not by the user.{{% /alert %}}
